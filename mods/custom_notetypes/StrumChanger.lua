function onCreatePost()
	for i = 0, getProperty('unspawnNotes.length') - 1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'StrumChanger' then

			setPropertyFromGroup('unspawnNotes', i,'texture','StrumChanger')
			if not sus then
...
function onUpdatePost(el)
    ...
    for a = 0, getProperty('notes.length') - 1 do
        ...
        if getPropertyFromGroup('notes', a, 'noteType') == 'StrumChanger' then
