...
function updateShadow()
	...
	for i = 0, getProperty('dad.animationsArray.length')-1 do
		if getPropertyFromGroup('dad.animationsArray', i, 'anim') == 'singLEFT' then
			anim[1] = getPropertyFromGroup('dad.animationsArray', i, 'name')
			ofs[1] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[1]
			ofs[2] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[2]
			fps[1] = getPropertyFromGroup('dad.animationsArray', i, 'fps')
		end
		if getPropertyFromGroup('dad.animationsArray', i, 'anim') == 'singDOWN' then
			anim[2] = getPropertyFromGroup('dad.animationsArray', i, 'name')
			ofs[3] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[1]
			ofs[4] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[2]
			fps[2] = getPropertyFromGroup('dad.animationsArray', i, 'fps')
		end
		if getPropertyFromGroup('dad.animationsArray', i, 'anim') == 'singUP' then
			anim[3] = getPropertyFromGroup('dad.animationsArray', i, 'name')
			ofs[5] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[1]
			ofs[6] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[2]
			fps[3] = getPropertyFromGroup('dad.animationsArray', i, 'fps')
		end
		if getPropertyFromGroup('dad.animationsArray', i, 'anim') == 'singRIGHT' then
			anim[4] = getPropertyFromGroup('dad.animationsArray', i, 'name')
			ofs[7] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[1]
			ofs[8] = getPropertyFromGroup('dad.animationsArray', i, 'offsets')[2]
			fps[4] = getPropertyFromGroup('dad.animationsArray', i, 'fps')
		end
	end
end
...
function onUpdate()
	...
	colorArray = getProperty("dad.healthColorArray")
...
function opponentNoteHit(a,b,c,d)
	if c ~= 'GF Sing' or not gfSection then
	if b == 0 then
		left = 1
	...
	if shadow and shadowAlpha > 1.9 and (d == false or (d == true and cooldown[b+1])) then
		local tag = 'dadshadow'..tostring(i)
...
function onTweenCompleted(tag)
	if startwith(tag, 'dadshadow') then
