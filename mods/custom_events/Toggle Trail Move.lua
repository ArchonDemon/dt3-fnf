...
trailLength = 8; --8
trailDelay = 0.05; --0.05
numberDad = 1 --0.4
numberBF = 1 --0.4
function onEvent(name, value1, value2)
	if name == "Toggle Trail Move" then
		...
		if not (value1 == '0' or value1 == '') then
			...
			function createTrailFrameDad(tag)
				...
				if not (image == '') then
					trailTagD = 'psychicTrail'..tag..num;
					makeAnimatedLuaSprite(trailTagD, image, x, y);
					setProperty(trailTagD..'.offset.x', offsetX);
					setProperty(trailTagD..'.offset.y', offsetY);
					setProperty(trailTagD..'.flipX', flip)
					setProperty(trailTagD..'.scale.x', scaleX)
					setProperty(trailTagD..'.scale.y', scaleY)
					--setObjectOrder(trailTagD, objectOrder)
					setProperty(trailTagD..'.alpha', numberDad);
					setProperty(trailTagD..'.color', color);
					--setBlendMode(trailTagD, 'add');
					addAnimationByPrefix(trailTagD, 'stuff', frame, 0, false);
					addLuaSprite(trailTagD, false);

					function opponentNoteHit(id, direction, noteType, isSustainNote)
						if direction == 0 then
							doTweenX(trailTagD, trailTagD, getProperty('dad.x') -move, numberDad, 'circOut')
						end
						if direction == 1 then
							doTweenY(trailTagD, trailTagD, getProperty('dad.y') +move, numberDad, 'circOut')
						end
						if direction == 2 then
							doTweenY(trailTagD, trailTagD, getProperty('dad.y') -move, numberDad, 'circOut')
						end
		...
		function createTrailFrameBF(tag)
			...
			if not (image == '') then
				...
				function goodNoteHit(id, direction, noteType, isSustainNote)
					if direction == 0 then
						doTweenX(trailTag, trailTag, getProperty('boyfriend.x') -move, numberBF, 'circOut')
					end
					if direction == 1 then
						doTweenY(trailTag, trailTag, getProperty('boyfriend.y') +move, numberBF, 'circOut')
					end
					if direction == 2 then
						doTweenY(trailTag, trailTag, getProperty('boyfriend.y') -move, numberBF, 'circOut')
					end
